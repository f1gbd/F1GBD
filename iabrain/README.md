...
[![Version](https://img.shields.io/badge/version-iabrain--v1.51.00-blue)](https://github.com/f1gbd/F1GBD/releases/tag/iabrain-v1.51.00)
...
### 📥 [**Télécharger la dernière version**](https://github.com/f1gbd/F1GBD/releases/download/iabrain-v1.51.00/IAbrain.7z)
...
<img src="docs/images/IAbrain_vibecoding.png" alt="Vibe Coding avec IAbrain" width="1024">
...
| 📁 | **Indexation d'un dossier complet — base perso & base ADRASEC** *(v1.50 / v1.51)* | Un panneau dédié indexe **récursivement un dossier entier et tous ses sous-dossiers**, en un clic. En v1.50 pour la base perso (menu **« 📁 RAG perso »**), en v1.51 aussi pour la **base de référence ADRASEC** (menu **Connaissances**, avec avertissement dédié). Prise en charge de tous les formats natifs (PDF, DOCX, TXT, MD, CSV…) **et conversion automatique des classeurs Excel** (`.xlsx/.xlsm`). **OCR automatique des PDF scannés et des Word image** (via Tesseract) : les documents image sans couche texte (conventions signées, arrêtés…) sont reconnus et indexés au lieu d'être rejetés. Indexation **incrémentale** (SHA-256), **purge** des fichiers disparus, **simulation** (dry-run) et **sauvegarde horodatée** avant écriture. Les deux bases restent strictement isolées. |
...
#### 📥 [**Télécharger IAbrain.7z**](https://github.com/f1gbd/F1GBD/releases/download/iabrain-v1.51.00/IAbrain.7z)

*(version `iabrain-v1.51.00` — voir [toutes les releases IAbrain](https://github.com/f1gbd/F1GBD/releases?q=iabrain) pour les versions précédentes)*
...
## 🆕 Évolution récente — v1.33 → v1.51

Les versions récentes ont apporté plusieurs améliorations majeures, du RAG hybride aux corrections manuelles, en passant par la cartographie, les macros utilisateur, la connectivité cloud, la mémoire conversationnelle, le profil opérateur, l'auto-exécution de macros par le LLM, le remplissage automatique du SITREP PDF officiel ADRASEC, l'exécution non-bloquante des macros Action avec feedback de progression en direct, la détection de négation contextuelle dans le plugin SITREP, l'interface vocale complète (reconnaissance + synthèse) avec apprentissage des corrections de l'opérateur, une barre de 24 macros colorées et partageables complétée par les actions natives POS OSM (carte géocodée IGN) et SITREP SATER (compte-rendu PDF), un **avatar IAbrain animé**, un **« vibe coding » Python fiabilisé** (panneau de code exportable, ré-indentation automatique, gestion automatique du contexte et détection de troncature), et désormais l'**indexation d'un dossier complet dans la base RAG personnelle** (avec **OCR automatique des PDF scannés**) via le menu « 📁 RAG perso » — panneau étendu en v1.51 à la **base de référence ADRASEC** (mêmes options, depuis le menu Connaissances).

### 🗂 v1.51.x — Panneau d'indexation commun : base perso **et** base ADRASEC

La v1.51 étend le panneau d'indexation riche (dossier complet + sous-dossiers, options Excel / OCR / force / purge / sauvegarde / simulation, journal en direct) — jusque-là réservé à la base perso — à la **base de référence ADRASEC**. Le menu **Connaissances → « Indexer / mettre à jour la base (dossier)… »** (et « Réindexer entièrement (force) ») ouvre désormais **le même panneau**, clairement identifié comme base de référence (avertissement « base publiée sur GitHub »), avec le dossier de connaissances pré-rempli. Résultat : la base ADRASEC bénéficie exactement des mêmes fonctions que la perso, **conversion Excel et OCR des documents scannés compris**. Le suivi reste consigné dans `rag_meta.json`, si bien que « Documents indexés », les statistiques et le pack de publication demeurent cohérents.

| Version | Apport principal |
|---|---|
| **1.51.00** | **Panneau d'indexation unifié** perso / **base ADRASEC** : même présentation et mêmes options (Excel, **OCR des scans**, force, purge, sauvegarde, dry-run, découpage) pour construire la base de référence via le menu Connaissances. Isolation stricte des deux bases ; suivi `rag_meta.json` pour la base principale (compatible pack de publication). |
