...
![Interface MeshRNS (Raspberry Pi)](https://raw.githubusercontent.com/f1gbd/F1GBD/master/meshrns/raspberry/images/MeshRNSpi_mainscreen.png)

> Version courante : **v2.1.11** — **Raspberry Pi OS 64 bits** (Pi 4B+/5) **et Linux x86-64** (Debian / Ubuntu 24.04…), interface graphique. Nouveau : **commandes de canal** `#?` `#v` `#p` `#h` (réponses **sur le canal**) et **routage de message vers email** `#m` (radiogramme authentifié).
### 📥 [**Télécharger la dernière version (Linux / Raspberry Pi)**](https://github.com/f1gbd/F1GBD/releases) — tag **`meshrns-v2.1.11`**

*Archives **`.tar.gz`** : `…-linux-aarch64.tar.gz` (Raspberry Pi 64 bits) ou `…-linux-x86_64.tar.gz` (PC/serveur). Détails dans « Installation » ci-dessous.*
...
- Un **companion MeshCore** (nœud LoRa) connecté en **USB** (apparaît en `/dev/ttyACM0` ou `/dev/ttyUSB0`), ou en **TCP** / **BLE**, avec le **canal de service `tcq`** créé (même **nom** *et* même **clé** que les stations du réseau TCQ).
...
Aucune source ni compilation : l'archive **`.tar.gz`** contient un **binaire autonome** (Python, Tkinter et la pile Reticulum/LXMF sont embarqués).

### 📥 Télécharger la dernière version

`https://github.com/f1gbd/F1GBD/releases` → tag **`meshrns-v2.1.11`**, puis l'archive correspondant à votre machine :

| Plateforme | `uname -m` | Archive |
| --- | --- | --- |
| Raspberry Pi 4B+/5 (64 bits) | `aarch64` | **`MeshRNS-v2.1.11-linux-aarch64.tar.gz`** |
| PC / serveur Linux | `x86_64` | **`MeshRNS-v2.1.11-linux-x86_64.tar.gz`** |

```bash
# 1. Extraction (adaptez le nom d'archive à votre plateforme)
tar xzf MeshRNS-v2.1.11-linux-aarch64.tar.gz
cd MeshRNS

# 2. Configuration système (accès série 'dialout', Bluetooth optionnel, raccourci menu)
chmod +x install_pi.sh
./install_pi.sh
#   -> si le script vous a ajouté au groupe 'dialout', déconnectez/reconnectez la session.

# 3. Lancement
./MeshRNS                          # interface graphique
# ou, en console / headless :
./MeshRNS --nogui -c meshrns.json
```

> **Contenu de l'archive** : le binaire `MeshRNS` + son dossier `_internal/`, les scripts **`install_pi.sh`** (configuration système) et **`autostart_pi.sh`** (démarrage automatique), les icônes `MeshRNS.png` / `MeshRNS.ico`, le dossier `reticulum_config/` (config RNS par défaut) et un `meshrns.json` par défaut (**SMTP vierge**, port à adapter). L'annuaire `annuaire.json` est créé/maintenu à côté du binaire — placez le dossier dans un emplacement **inscriptible** (votre home, pas un dossier système). Au 1ᵉʳ lancement, si `~/.reticulum/config` est absent, `reticulum_config/config` y est copié automatiquement, et `meshrns.json` est créé s'il manque.

### Vérifier l'intégrité (SHA-256)

```bash
sha256sum -c MeshRNS-v2.1.11-linux-aarch64.tar.gz.sha256
```
...
3. **MeshCore (companion)** : choisissez le **Transport** (`serial`/`tcp`/`ble`), le **Port** (ex. `/dev/ttyACM0` ou `/dev/ttyUSB0`) et le **Baudrate**. Laissez **Lecture active (get_msg)** cochée.
...
> **Port série sous Linux** : réglez le **Port** sur `/dev/ttyACM0` (USB-CDC) ou `/dev/ttyUSB0` (adaptateur série). Si le `meshrns.json` fourni contient un port **Windows** (`COMx`), remplacez-le. Idéalement, utilisez le chemin **stable** `/dev/serial/by-id/…` (`ls -l /dev/serial/by-id/`), insensible à l'ordre de branchement USB.
...
{
  "meshcore": {
    "transport": "serial", "port": "/dev/ttyACM0", "baudrate": 115200,
...
| `annuaire_file` | Fichier de l'annuaire TCQ (défaut `annuaire.json`, à côté du binaire). |
...
## Recommandations (Linux / Raspberry Pi)

- **Port série stable** — préférez `/dev/serial/by-id/…` à `/dev/ttyUSB0`/`/dev/ttyACM0` (insensible à l'ordre de branchement). Remplacez tout port Windows (`COMx`) hérité du `meshrns.json` publié.
- **Droits série** — `./install_pi.sh` ajoute l'utilisateur au groupe `dialout` (ou `sudo usermod -aG dialout $USER`), puis **reconnectez la session**.
- **Dossier inscriptible** — gardez l'application dans votre home pour que `meshrns.json`, `annuaire.json` et les journaux persistent.
- **Un seul accès au port** — n'ouvrez pas l'interface graphique **et** un service headless en même temps : un seul programme peut tenir le port série.
- **Cycle Démarrer / Arrêter** — le port série est libéré à l'arrêt et l'ouverture est réessayée au démarrage ; redémarrer sans quitter l'application fonctionne. L'exécutable intègre un **verrou anti-double-lancement**.
- **Duty cycle EU868 (1 %)** — limitez les canaux miroités au seul canal de service `tcq`.

---

## Lancement automatique au démarrage

Pour qu'une passerelle ADRASEC démarre **automatiquement** à l'allumage (poste avec bureau, Raspberry Pi ou PC Linux) :

```bash
chmod +x autostart_pi.sh
./autostart_pi.sh          # active MeshRNS au démarrage (autostart XDG)
sudo reboot
```

Pour **revenir en arrière** : `./autostart_pi.sh --undo` puis `sudo reboot`. L'autostart ouvre l'**interface** ; cliquez sur **Démarrer** pour lancer la passerelle (ou activez `auto_start` dans `meshrns.json` pour un démarrage sans surveillance).
...
- **`/dev/ttyACM0` (ou `ttyUSB0`) : permission refusée** → vous n'êtes pas dans le groupe `dialout` : lancez `./install_pi.sh` puis reconnectez la session.
- **Companion non détecté** → `ls /dev/ttyACM* /dev/ttyUSB*` et ajustez le **Port**.
- **La passerelle ne s'arrête pas / impossible de redémarrer** → corrigé (arrêt borné, **port série libéré** à l'arrêt et **réessai d'ouverture** au démarrage, client LXMF persistant). Assurez-vous d'être en **v2.1.11**.
