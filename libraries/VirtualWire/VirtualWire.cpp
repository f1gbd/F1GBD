...
//	Define digitalRead, digitalWrite and digital pins for Arduino like platforms
	...
	static uint8_t vw_tx_pin = 12; //12;
...
static uint8_t vw_tx_buf[(VW_MAX_MESSAGE_LEN * 2) + VW_HEADER_LEN] 
	= {0x00, 0xa0, 0xaa, 0xaa, 0xaa, 0x6a, 0x9a, 0x55};   // Trame406 ALARM: (8 bits) Attention, envoyé de droite à gauche !!!
...
/*
    ...
    */
	...
	else
	{
	    ...
	    if (vw_tx_bit >= 8)	// 6 :transmission en 6 bits
	    {
		vw_tx_bit = 0;
		vw_tx_index++;
	    }
    ...
    if (vw_tx_sample >= 8)			// 6: transmission en 6 bits
	vw_tx_sample = 0;
	/*
