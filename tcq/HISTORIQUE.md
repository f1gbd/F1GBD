# TCQ — historique détaillé des versions

> Cette page conserve le détail de chaque version, dans l'ordre inverse.
> Pour découvrir TCQ, commencez plutôt par le **[README](README.md)** : il
> présente l'application, ce qu'elle sait faire et comment l'installer, avec un
> journal synthétique des dernières mises à jour.
>
> [⬅ Retour au README](README.md) · [📜 Toutes les releases](https://github.com/f1gbd/F1GBD/releases?q=tcq)
...
### 🔌 Partir en mission sans réseau

Le calcul est local ; les altitudes, non. Et le cache de points est indexé au
point **exact** (≈ 10 m) : sur le terrain on clique où l'on se trouve, jamais
où l'on avait cliqué la veille — il ne resservira pas.

**Avant de partir**, deux gestes :

1. **⬇️ Précharger** — les tuiles OpenStreetMap de la zone (bouton existant).
2. **📡 LoRa → ⚙ → ⬇️ Préparer le relief de la zone affichée** — une grille
   d'altitudes est téléchargée sur l'écran courant plus une marge, et conservée.

| Zone | Pas 200 m | Durée | Sur le disque |
|---|---|---|---|
| 25 × 32 km | 20 700 points | ≈ 4 min (208 appels) | ≈ 360 Ko |
| 12 × 16 km | 4 800 points | ≈ 1 min (49 appels) | ≈ 85 Ko |

Hors ligne, couverture **et** recherche de relais fonctionnent alors sur cette
zone, par interpolation.

> **La marge de 7 km n'est pas de la précaution.** Le corridor exploré pour un
> relais déborde de **plus de 6 km** de part et d'autre de l'axe A–B. Préparer
> pile le rectangle qui joint A et B laisserait la recherche échouer sur les
> bords, sans réseau pour la sauver.

Une altitude interpolée n'est pas une altitude mesurée : la légende, l'infobulle
et le journal écrivent **« Relief INTERPOLÉ — grille 200 m »** en orange. Et un
point hors de toute zone préparée ne reçoit **aucune** altitude — la recherche
échoue en disant combien de points manquent, plutôt que de combler les trous
avec un relief plausible et faux.
...
✅ Vérifié par **76 contrôles** sur le moteur, **24 sur le mode hors ligne** et
**100 contrôles d'interface** sous Tk réel, sur quatre tailles d'écran
(800×600 → 1920×1080).
